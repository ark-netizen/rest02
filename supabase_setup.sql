...
-- author_id 를 public.profiles(id) 로 참조해야 PostgREST join 이 동작함
...
create policy "notices: 누구나 조회 가능"
  on public.notices for select
  using (true);

create policy "notices: 관리자만 등록 가능"
  on public.notices for insert
  with check (
    exists (select 1 from public.profiles where id = auth.uid() and is_admin = true)
  );

create policy "notices: 관리자만 수정 가능"
  on public.notices for update
  using (
    exists (select 1 from public.profiles where id = auth.uid() and is_admin = true)
  );

create policy "notices: 관리자만 삭제 가능"
...
create policy "posts: 누구나 조회 가능"
  on public.posts for select
  using (true);

create policy "posts: 로그인 사용자 등록 가능"
  on public.posts for insert
  with check (auth.uid() is not null);

create policy "posts: 본인 또는 관리자만 수정 가능"
...
create policy "posts: 본인 또는 관리자만 삭제 가능"
...
create policy "qna: 공개 글은 누구나, 비공개는 본인/관리자만 조회"
...
create policy "qna: 로그인 사용자 등록 가능"
  on public.qna for insert
  with check (auth.uid() is not null);

create policy "qna: 본인 또는 관리자만 수정 가능"
...
create policy "qna: 본인 또는 관리자만 삭제 가능"
...
-- 익명/로그인 사용자 모두 조회수 함수 호출 허용
grant execute on function public.increment_post_views(text, bigint) to anon, authenticated;


-- ⑥ 관리자 계정 지정 (회원가입 후 아래 쿼리 별도 실행)
-- update public.profiles
--   set is_admin = true
--   where id = (select id from auth.users where email = 'ark230015@gmail.com');
